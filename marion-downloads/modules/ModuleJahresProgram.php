...
class ModuleDownloadJP
{
	...
	public static function get_downloads($dataBase)
	{
		...
		$objAus = $dataBase->prepare("SELECT * FROM  tl_downloads WHERE (type = 'Jahresprogram') ORDER BY tstamp DESC")->limit(1)->execute(time());
		$count=0;
		while ($objAus->next()) {
			++$count;
			$var1;
...
/**
 * Class ModuleJahresProgram
 */
 
class ModuleJahresProgram extends Module
{
