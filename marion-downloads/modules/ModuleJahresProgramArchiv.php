...
class ModuleDownloadJPArchiv
{
	...
	public static function get_downloads($dataBase)
	{
		...
		$objAus = $dataBase->execute("SELECT * FROM  tl_downloads WHERE (type = 'Jahresprogram') ORDER BY tstamp DESC");
		while ($objAus->next()) {
			$var1;
			$objHref = null;
			if($objAus->href != '')
...
/**
 * Class ModuleJahresProgramArchiv
 */
class ModuleJahresProgramArchiv extends Module
{
